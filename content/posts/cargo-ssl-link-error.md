...
while developing a side project I required some depenecies, the dependencies in intrust are [jwt-simple](https://crates.io/crates/jwt-simple), and [reqwest](https://crates.io/crates/reqwest)

while developing everything worked fine, my app was build successfully (didn't test _release_ build) and no issues, I needed to delete
my _/target_ file that cargo creates, then after coming back to continue development of the project I faced this scary error
```sh
...
a linkage error, recenetly I updated my system, maybe `libssl` was also update with *BREAKING CHANGES?* doesn't make sense, but I checked
...
is the cause of the issue, now the question is who intruduced this `boring-sys` crate?

after a quick search in `Cargo.lock` turns out it was the `jwt-simple` crate, luckly someone before me already faced it or the maintainer just knew it
